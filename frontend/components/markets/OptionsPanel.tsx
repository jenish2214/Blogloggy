...
function ChainTable({ contracts, spot, type }: { contracts: OptionsContract[]; spot: number; type: "call" | "put" }) {
  return (
    <table className="data-table">
      ...
      <tbody>
        {contracts.map((c) => (
          <OptionsRow key={c.contractSymbol} contract={c} spot={spot} type={type} />
        ))}
        {contracts.length === 0 && (
          <tr><td colSpan={9} style={{ padding: 24, textAlign: "center", color: "var(--text-muted)", fontFamily: "var(--font-mono)" }}>No contracts available</td></tr>
        )}
      </tbody>
    </table>
  );
...
export default function OptionsPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  const [tab, setTab] = useState<"all" | "calls" | "puts">("all");
  const [selectedExpiry, setSelectedExpiry] = useState<number | null>(null);
  const [lastUpdated, setLastUpdated] = useState<Date | null>(null);
  const chainRef = useRef<{ sym: string; expiry: number | null }>({ sym: "AAPL", expiry: null });

  const loadChain = useCallback(async (sym: string, expiry?: number, silent = false) => {
    ...
    try {
      ...
      chainRef.current = { sym, expiry: expiry ?? data.expirations?.[0] ?? null };
      if (!expiry && data.expirations?.[0]) setSelectedExpiry(data.expirations[0]);
    } catch (e) {
      if (!silent) {
        const msg = e instanceof Error ? e.message : "";
        setError(msg.includes("no_options") || msg.includes("404")
          ? "No options chain for this symbol. Try US symbols: AAPL, MSFT, NVDA, TSLA, SPY."
          : "Failed to load options chain. Try AAPL, MSFT, NVDA, TSLA.");
      }
    }
  ...
  useEffect(() => { loadChain("AAPL"); }, [loadChain]);

  useEffect(() => {
    if (!chain) return;
    const id = setInterval(() => {
      const { sym, expiry } = chainRef.current;
      if (sym) loadChain(sym, expiry ?? undefined, true);
    }, REFRESH_MS);
  ...
  return (
    <div className={embedded ? undefined : "page"}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
        {["AAPL", "MSFT", "NVDA", "TSLA", "SPY", "QQQ", "AMZN", "META"].map((s) => (
          <button
            key={s}
            type="button"
            className="btn btn-ghost btn-sm"
            onClick={() => { setInputSym(s); setSymbol(s); loadChain(s); }}
          >
            {s}
          </button>
        ))}
        ...
        <div className="card">
          ...
          <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
            <div>
              ...
              <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.72rem", color: "var(--text-muted)", marginLeft: 10 }}>
                {totalContracts} contracts
              </span>
            </div>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontFamily: "var(--font-mono)", fontSize: "0.68rem", color: "var(--up)", textTransform: "uppercase", letterSpacing: "0.08em" }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--up)", animation: refreshing ? "pulse 1s ease infinite" : "none" }} />
            ...
            <button type="button" className="btn btn-ghost btn-sm" disabled={loading || refreshing} onClick={() => loadChain(chain.symbol, selectedExpiry ?? undefined, true)}>
          ...
          <div className="tabs" style={{ padding: "0 16px" }}>
            <button className={`tab-btn ${tab === "all" ? "active" : ""}`} onClick={() => setTab("all")}>All ({totalContracts})</button>
            <button className={`tab-btn ${tab === "calls" ? "active" : ""}`} onClick={() => setTab("calls")}>Calls ({calls.length})</button>
            <button className={`tab-btn ${tab === "puts" ? "active" : ""}`} onClick={() => setTab("puts")}>Puts ({puts.length})</button>
          </div>

          <div style={{ overflowX: "auto", maxHeight: tab === "all" ? undefined : "70vh", overflowY: tab === "all" ? undefined : "auto" }}>
            {tab === "all" ? (
              <>
                <div style={{ padding: "10px 16px", fontFamily: "var(--font-mono)", fontSize: "0.72rem", color: "var(--up)", fontWeight: 600, borderBottom: "1px solid var(--border)" }}>
                  CALLS — {calls.length} contracts
                </div>
                <ChainTable contracts={calls} spot={chain.spotPrice} type="call" />
                <div style={{ padding: "10px 16px", fontFamily: "var(--font-mono)", fontSize: "0.72rem", color: "var(--down)", fontWeight: 600, borderBottom: "1px solid var(--border)", borderTop: "1px solid var(--border)" }}>
                  PUTS — {puts.length} contracts
                </div>
                <ChainTable contracts={puts} spot={chain.spotPrice} type="put" />
              </>
            ) : (
              <ChainTable
                contracts={tab === "calls" ? calls : puts}
