...
export function OrderForm({ symbol, name, assetClass, currentPrice, onSuccess }: Props) {
  ...
  const { placeOrder, cash, positions } = usePortfolioStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Try server-side order (persists to Supabase), fallback to local store
    try {
      const { portfolioApi } = await import("@/lib/api");
      const serverRes = await portfolioApi.placeOrder({
        symbol, name, assetClass, side, qty: q, orderType,
        filledPrice: fillPrice, limitPrice: orderType === "limit" ? parseFloat(limitPrice) : undefined,
      });
      setResult(serverRes);
      if (serverRes.success) { setQty(""); onSuccess?.(serverRes.message); }
      return;
    } catch {
      // Fallback to local store if not authenticated
    }

    const res = placeOrder({
      symbol, name, assetClass, side, qty: q, orderType, currentPrice,
      limitPrice: orderType === "limit" ? parseFloat(limitPrice) : undefined,
    });
    setResult(res);
    if (res.success) { setQty(""); onSuccess?.(res.message); }
  };
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {[25, 50, 75, 100].map((pct) => (
            <button
              ...
              onClick={() => {
                ...
                setQty(((maxQty * pct) / 100).toFixed(assetClass === "crypto" ? 4 : 0));
              }}
            >
              {pct}%
            </button>
      ...
      {/* Order summary */}
      {total > 0 && (
        <div style={{ background: "var(--bg-surface-2)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "8px 12px", display: "flex", justifyContent: "space-between" }}>
          <div>
      ...
      >
        {side === "buy" ? "▲" : "▼"} Place {orderType.toUpperCase()} {side.toUpperCase()} Order
      </button>
