...
export function Navbar() {
  ...
  return (
    <header style={{ position: "fixed", top: 0, left: 0, right: 0, height: "var(--navbar-h)", background: "rgba(5,5,8,0.96)", backdropFilter: "blur(12px)", borderBottom: "1px solid var(--border)", zIndex: 1000, display: "flex", alignItems: "center", padding: "0 20px", gap: 20 }}>
      {/* Logo */}
      <Link href="/" style={{ display: "flex", alignItems: "center", gap: 8, textDecoration: "none", flexShrink: 0 }}>
        ...
        <span style={{ fontFamily: "var(--font-mono)", fontWeight: 700, fontSize: "0.88rem", color: "var(--text-primary)", letterSpacing: "0.04em" }}>
          QUANT<span style={{ color: "var(--accent-2)" }}>DESK</span>
        </span>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.58rem", color: "var(--text-muted)", background: "var(--accent-soft)", border: "1px solid rgba(99,102,241,0.2)", padding: "1px 4px", borderRadius: 2 }}>
          PAPER
        </span>
      </Link>
      ...
      <nav style={{ display: "flex", gap: 0, flex: 1 }} className="hide-mobile">
        {NAV_LINKS.map(({ href, label }) => (
          <Link key={href} href={href} style={{ fontFamily: "var(--font-mono)", fontSize: "0.68rem", fontWeight: 500, letterSpacing: "0.08em", color: path === href ? "var(--text-primary)" : "var(--text-muted)", textDecoration: "none", padding: "4px 10px", borderRadius: 2, borderBottom: path === href ? "1px solid var(--accent)" : "1px solid transparent", transition: "var(--t-fast)" }}>
            {label}
          </Link>
        ))}
      </nav>
          ...
          {user ? (
            <>
              ...
              <Link href="/messages" style={{ position: "relative", textDecoration: "none", display: "flex", alignItems: "center" }}>
                <div style={{ padding: "4px 8px", background: unread > 0 ? "var(--accent-soft)" : "transparent", border: "1px solid " + (unread > 0 ? "rgba(99,102,241,0.25)" : "var(--border)"), borderRadius: 2, cursor: "pointer" }}>
              ...
              {/* Avatar + menu */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "3px 8px 3px 3px", background: "var(--bg-surface)", border: "1px solid var(--border-strong)", borderRadius: 2 }}>
                <div style={{ width: 24, height: 24, borderRadius: 2, background: "var(--accent-soft)", border: "1px solid rgba(99,102,241,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-mono)", fontSize: "0.62rem", fontWeight: 700, color: "var(--accent-2)" }}>
                  {initials}
                </div>
      ...
      {menuOpen && (
        <div style={{ position: "fixed", top: "var(--navbar-h)", left: 0, right: 0, background: "var(--bg-surface)", borderBottom: "1px solid var(--border)", padding: 16, display: "flex", flexDirection: "column", gap: 4, zIndex: 999 }}>
          {NAV_LINKS.map(({ href, label }) => (
            <Link key={href} href={href} onClick={() => setMenuOpen(false)} style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem", fontWeight: 500, letterSpacing: "0.08em", color: path === href ? "var(--accent-2)" : "var(--text-secondary)", textDecoration: "none", padding: "10px 12px", borderRadius: 2, background: path === href ? "var(--accent-soft)" : "transparent" }}>
              {label}
            </Link>
          ))}
          {mounted && user ? (
