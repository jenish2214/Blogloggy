      ...
      <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border-strong)", borderRadius: "var(--radius)", overflow: "hidden" }}>
        ...
        {success ? (
          <div style={{ padding: 32, textAlign: "center" }}>
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.9rem", fontWeight: 600, color: "var(--up)", marginBottom: 8 }}>
              Account created!
            </div>
            <div style={{ fontFamily: "var(--font-sans)", fontSize: "0.82rem", color: "var(--text-secondary)" }}>
          ...
          <form onSubmit={handleSignup} style={{ padding: "12px 24px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <label style={{ fontFamily: "var(--font-mono)", fontSize: "0.72rem", color: "var(--text-muted)", display: "block", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.08em" }}>Display Name</label>
            ...
            <div>
              <label style={{ fontFamily: "var(--font-mono)", fontSize: "0.72rem", color: "var(--text-muted)", display: "block", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.08em" }}>Email Address</label>
            ...
            <div>
              <label style={{ fontFamily: "var(--font-mono)", fontSize: "0.72rem", color: "var(--text-muted)", display: "block", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.08em" }}>Password</label>
              ...
              <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.72rem", color: "var(--text-muted)", marginTop: 4 }}>
                Strength: {password.length === 0 ? "—" : password.length < 8 ? "⚠ too short" : password.length < 12 ? "▪ moderate" : "▪▪ strong"}
              </div>
            </div>

            {error && (
              <div style={{ padding: "8px 12px", background: "var(--down-soft)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: "var(--radius-sm)", fontFamily: "var(--font-mono)", fontSize: "0.75rem", color: "var(--down)" }}>
                ✗ {error}
              </div>
            )}
            ...
            {/* What you get */}
            <div style={{ padding: "12px 14px", background: "var(--bg-surface-2)", borderRadius: "var(--radius-sm)", display: "flex", flexDirection: "column", gap: 6 }}>
              {["$100,000 paper capital instantly", "Live stock & crypto prices", "Options chain + Greeks", "Backtest any strategy", "Trade history persisted to cloud"].map((f) => (
                <div key={f} style={{ fontFamily: "var(--font-mono)", fontSize: "0.7rem", color: "var(--text-secondary)", display: "flex", gap: 8 }}>
