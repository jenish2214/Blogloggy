...
function EquityChart({ curves, showAll }: { curves: { label: string; data: { time: number; value: number }[]; color: string }[]; showAll: boolean }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", display: "block" }}>
      ...
      {ticks.map((t) => (
        <g key={t}>
          <line x1={PAD.left} y1={toY(t)} x2={W - PAD.right} y2={toY(t)} stroke="rgba(255,255,255,0.04)" strokeWidth={0.5} />
          <text x={PAD.left - 6} y={toY(t) + 4} textAnchor="end" fill="#555568" style={{ fontSize: 10, fontFamily: "IBM Plex Mono, monospace" }}>
            ${(t / 1000).toFixed(0)}k
          </text>
        </g>
      ))}
      ...
      {/* $100k baseline */}
      <line x1={PAD.left} y1={toY(100_000)} x2={W - PAD.right} y2={toY(100_000)} stroke="rgba(255,255,255,0.15)" strokeWidth={1} strokeDasharray="5,5" />
      <text x={PAD.left - 6} y={toY(100_000) - 4} textAnchor="end" fill="#8888a0" style={{ fontSize: 9, fontFamily: "IBM Plex Mono, monospace" }}>BASE</text>

      {/* X labels */}
      {xIdx.map((i) => (
        <text key={i} x={toX(i, primary.length)} y={H - 6} textAnchor="middle" fill="#555568" style={{ fontSize: 9, fontFamily: "IBM Plex Mono, monospace" }}>
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", display: "block" }}>
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            <rect x={x} y={y} width={bW} height={bH} fill={isNeg ? "rgba(248,113,113,0.55)" : "rgba(99,102,241,0.55)"} rx={1} style={{ transformOrigin: `${x}px ${H - PAD.bottom}px`, animation: `expandBar 0.6s cubic-bezier(0.22,1,0.36,1) ${i * 20}ms both` }} />
          </g>
        );
      })}
...
/* ── Strategy colors ─────────────────────────────────────────────────────── */
const STRATEGY_COLORS = ["var(--accent-2)", "var(--up)", "var(--warn)", "#a78bfa"];
const PRESETS = ["AAPL", "TSLA", "NVDA", "MSFT", "SPY", "QQQ", "BTC-USD", "AMZN", "GOOGL", "META"];
...
export default function ResearchPage() {
  ...
  return (
    <div className="page" style={{ maxWidth: 1280 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {PRESETS.map((p) => (
            <button
              ...
              onClick={() => setSymbol(p)}
              style={{
                padding: "5px 12px", fontFamily: "var(--font-mono)", fontSize: "0.78rem", cursor: "pointer",
                background: symbol === p ? "var(--accent-soft)" : "transparent",
                color: symbol === p ? "var(--accent-2)" : "var(--text-muted)",
                border: `1px solid ${symbol === p ? "rgba(99,102,241,0.3)" : "var(--border)"}`,
      ...
      {error && (
        <div className="anim-fade-up" style={{ padding: "12px 16px", background: "var(--down-soft)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: "var(--radius-sm)", marginBottom: 20, fontFamily: "var(--font-mono)", fontSize: "0.85rem", color: "var(--down)" }}>
          ✗ {error}
        </div>
      )}
              ...
              {activeTab === "distribution" && result.distribution && (
                <>
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.75rem", color: "var(--text-muted)", marginBottom: 12 }}>
                    Daily return distribution — <span style={{ color: "rgba(248,113,113,0.8)" }}>■</span> negative days &nbsp;
                    <span style={{ color: "rgba(99,102,241,0.8)" }}>■</span> positive days
                  </div>
